...
use tauri::{
    ...
    Emitter, Manager, Monitor, PhysicalPosition, PhysicalSize, Runtime, State, Window, WindowEvent,
};
...
#[derive(Default)]
struct DesktopLyricBoundsSaveState {
...
fn schedule_desktop_lyric_bounds_save<R: Runtime>(window: &Window<R>) {
...
fn persist_desktop_lyric_bounds_in_background<R: Runtime>(window: &Window<R>) {
    let Some(bounds) = capture_desktop_lyric_bounds(window) else {
        return;
    };
    let app_handle = window.app_handle().clone();
...
mod tests {
    ...
    fn desktop_lyric_bounds_outside_all_monitors_are_clamped_to_fallback() {
        ...
        let bounds = lyric_bounds(5000, 4000, 1400, 500, Some(1.0));

        let normalized = normalize_desktop_lyric_bounds(bounds, &[primary], Some(primary)).unwrap();
    ...
    fn desktop_lyric_bounds_use_overlapping_monitor_and_preserve_logical_size() {
        ...
        let bounds = lyric_bounds(2000, 100, 600, 200, Some(1.0));

        let normalized =
            normalize_desktop_lyric_bounds(bounds, &[primary, high_dpi], Some(primary)).unwrap();
    ...
    fn desktop_lyric_bounds_are_capped_to_the_visible_work_area() {
        ...
        let bounds = lyric_bounds(-1600, -200, 4000, 2000, None);
