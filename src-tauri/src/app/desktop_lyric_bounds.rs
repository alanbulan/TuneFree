...
#[path = "__tests__/lyric_bounds.rs"]
...
#[derive(Default)]
struct BoundsSaveQueue {
    pending: Option<PendingBoundsSave>,
    worker_active: bool,
}
...
pub(crate) struct DesktopLyricBoundsSaveState {
    ...
    queue: parking_lot::Mutex<BoundsSaveQueue>,
}
...
/// Debounces bounds persistence: move/resize events fire at refresh rate, so
/// each event only refreshes the pending entry and at most one waiter task is
/// alive at any moment.
pub(crate) fn schedule_desktop_lyric_bounds_save<R: Runtime>(window: &Window<R>) {
    ...
    let deadline = tokio::time::Instant::now() + DESKTOP_LYRIC_SAVE_DEBOUNCE;
    let should_spawn = {
        let state = app_handle.state::<DesktopLyricBoundsSaveState>();
        ...
        !std::mem::replace(&mut queue.worker_active, true)
    };
    if should_spawn {
        tauri::async_runtime::spawn(run_desktop_lyric_bounds_saver(app_handle));
    }
}

enum BoundsSaveStep {
    Sleep(tokio::time::Instant),
    Write(PendingBoundsSave),
    Exit,
}

fn next_bounds_save_step<R: Runtime>(app_handle: &tauri::AppHandle<R>) -> BoundsSaveStep {
    let state = app_handle.state::<DesktopLyricBoundsSaveState>();
    let mut queue = state.queue.lock();
    let now = tokio::time::Instant::now();
    if let Some(due) = queue.pending.take_if(|entry| now >= entry.deadline) {
        return BoundsSaveStep::Write(due);
    }
    match queue.pending.as_ref() {
        Some(entry) => BoundsSaveStep::Sleep(entry.deadline),
        None => {
            // worker_active is cleared under the same lock used to insert new
            // entries, so a concurrent schedule either sees the entry consumed
            // here or spawns a fresh worker after this one exits.
...
async fn run_desktop_lyric_bounds_saver<R: Runtime>(app_handle: tauri::AppHandle<R>) {
    loop {
        match next_bounds_save_step(&app_handle) {
            BoundsSaveStep::Exit => return,
            BoundsSaveStep::Sleep(deadline) => tokio::time::sleep_until(deadline).await,
            BoundsSaveStep::Write(entry) => {
                let write_app_handle = app_handle.clone();
                let result = tauri::async_runtime::spawn_blocking(move || {
                    persist_desktop_lyric_bounds_generation(
                        &write_app_handle,
                        &entry.bounds,
                        entry.generation,
                    )
...
pub(crate) async fn persist_desktop_lyric_webview_bounds<R: Runtime>(
    window: &tauri::WebviewWindow<R>,
) -> Result<(), String> {
    let Some(bounds) = capture_desktop_lyric_webview_bounds(window) else {
        return Ok(());
...
#[cfg(test)]
#[path = "__tests__/lyric_geometry.rs"]
mod tests;
