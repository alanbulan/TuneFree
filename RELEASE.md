...
构建源码来自指定的 `--ref`，其应用版本必须与 `tag` 一致。补包所需的平台适配提交可以晚于原标签，具体源码由 Actions 的 `head_sha` 追溯。目标 Release 必须已存在且为正式版本，已有 Mac 资产时默认拒绝覆盖。补发仍执行共享门禁和 Mac 验证，保留原 Windows 安装包；Tauri Action 合并更新清单中的 Mac 平台条目。
