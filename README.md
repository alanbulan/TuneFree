...
*   **使用中动态刷新**：播放、收藏、跳过、下载、推荐点击、曲库同步、不感兴趣反馈等行为写入后端后，会触发带去抖和最小间隔保护的后台刷新队列。`RECOMMENDATION_JOB_TTL_MS` 只表示任务状态在内存里保留多久，不再等同于推荐刷新频率。
...
flowchart TD
  ...
  I --> K[后台 worker 刷新推荐]
  B2[播放 / 收藏 / 跳过 / 下载 / 推荐点击 / 曲库同步] --> B3[动态刷新队列]
  B3 --> B4[20 秒去抖 / 3 分钟最小间隔 / 等待运行中 worker]
  B4 --> K
...
智能推荐 worker 不是固定 2 小时才执行一次。2 小时常量只用于保留内存里的任务状态，避免前端轮询任务时查不到刚完成的结果。真正的刷新触发来自两类入口：

| 触发来源 | 行为 |
| --- | --- |
| 应用启动 | 如果推荐开关开启，后端启动时创建新一轮 home 推荐任务；如果已有任务正在运行则复用运行中任务 |
| 播放行为 | `play_30s`、`play_complete`、`skip_early` 写入后触发动态刷新 |
| 收藏和歌单 | `favorite_add`、`favorite_remove`、`playlist_add` 写入后触发动态刷新 |
| 推荐反馈 | 推荐点击、不感兴趣、推荐反馈写入后触发动态刷新 |
| 曲库同步 | 收藏、歌单、队列、当前歌曲同步进 SQLite 后触发动态刷新 |

动态刷新有三层保护：

1. **20 秒去抖**：连续行为先合并，避免用户快速操作时每个事件都开一个云端 worker。
2. **3 分钟最小间隔**：限制云端调用频率，避免播放进度事件、收藏批量同步导致频繁请求。
3. **运行中等待**：如果上一轮 worker 还在跑，新触发不会丢弃，而是等待运行中任务结束后再补跑一轮。

因此实际体验是：用户使用越多，推荐画像越快更新；前端仍然只展示一份最新智能推荐结果。
