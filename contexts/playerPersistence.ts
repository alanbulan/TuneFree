...
export const loadStoredCurrentSong = (): Song | null => {
  ...
  return getStoredJson<Song | null>(
    PLAYER_STORAGE_KEYS.currentSong,
    null,
    normalizeSong,
  );
};
...
export const loadStoredPlayMode = (): PlayMode =>
  getStoredJson<PlayMode>(
    PLAYER_STORAGE_KEYS.playMode,
    DEFAULT_PLAY_MODE,
    normalizePlayMode,
  );
...
export const loadStoredAudioQuality = (): AudioQuality =>
  getStoredJson<AudioQuality>(
    PLAYER_STORAGE_KEYS.quality,
    DEFAULT_AUDIO_QUALITY,
    normalizeAudioQuality,
  );

/** 持久化前剥离运行时字段（签名地址、歌词等），重新打开时按身份重新解析。 */
...
export const persistPlayMode = (mode: PlayMode): void => {
  persistJson(
    PLAYER_STORAGE_KEYS.playMode,
    normalizePlayMode(mode) || DEFAULT_PLAY_MODE,
  );
};
...
export const persistAudioQuality = (quality: AudioQuality): void => {
  persistJson(
    PLAYER_STORAGE_KEYS.quality,
    normalizeAudioQuality(quality) || DEFAULT_AUDIO_QUALITY,
  );
};
